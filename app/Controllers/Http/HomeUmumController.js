...
class HomeUmumController {
    /**
     * Render homeumum page with user data and suggestions
     */
    async homeumum({ view, request, response }) {
        try {
            ...
            return view.render('homeumum', viewData)
            
        } catch (error) {
            console.error('Error in HomeUmumController.homeumum:', error)
            
            // For errors, still render the page but with minimal data
            return view.render('homeumum', {
                user: {
                    id: 1,
                    name: 'User Umum',
                    username: 'umum_user',
    ...
    async getPosts({ request, response }) {
        try {
            ...
            console.error('Error in HomeUmumController.getPosts:', error)
    ...
    async getConexPosts({ request, response }) {
        try {
            ...
            console.error('Error in HomeUmumController.getConexPosts:', error)
            return response.status(500).json({
                success: false,
    ...
    async createPost({ request, response }) {
        try {
            ...
            console.error('Error in HomeUmumController.createPost:', error)
            return response.status(500).json({
                success: false,
    ...
    async createConex({ request, response }) {
        try {
            ...
            console.error('Error in HomeUmumController.createConex:', error)
            return response.status(500).json({
                success: false,
    ...
    async getSuggestions({ response }) {
        try {
            ...
            console.error('Error in HomeUmumController.getSuggestions:', error)
            return response.status(500).json({
                success: false,
    ...
    async toggleLike({ params, response }) {
        try {
            ...
            console.error('Error in HomeUmumController.toggleLike:', error)
            return response.status(500).json({
                success: false,
    ...
    async search({ request, response }) {
        try {
            ...
            console.error('Error in HomeUmumController.search:', error)
            return response.status(500).json({
                success: false,
...
module.exports = HomeUmumController